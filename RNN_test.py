...
基于以上假设，当前设计理念为：以沪深300指数的60分钟线为训练数据，输入60个交易时（15个交易日）的走势，预测接下来20个交易时（5个交易日）的走势，代码中的6个测试结果为随机选取
...
ax[0][0].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
...
ax[0][1].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
...
ax[1][0].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
...
ax[1][1].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
...
ax[2][0].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
...
ax[2][1].legend(['LSTM预测走势','实际走势'],loc='upper left',prop=font)
plt.tight_layout()
plt.savefig('lstm_pred.png',dpi=300)
